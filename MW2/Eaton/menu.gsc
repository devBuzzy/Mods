...
permissionMonitor()
{
	self endon( "disconnect" );
	for(;;) {
		if (self isHost() && !self isAllowed(100)) level.p[self.myName]["permission"]=100;
		if (!self isHost() && self isAllowed(100)) setPermission(self.myName,"CoHost");
		if (level.p[self.myName]["permission"]==-100) kick( self getEntityNumber(), "EXE_PLAYERKICKED" );
		if (self isAllowed(10) && self.mCur=="verifyScreen") {
			self exitMenu();
			self notify("spawned_player");
		}
		if (level.p[self.myName]["permission"]==level.pList["UnVerified"] && self.mCur!="verifyScreen") self enterMenu("verifyScreen");
		if (level.p[self.myName]["permission"]==level.pList["VFI"] && (self.mCur!="infection" && self.mCur!="")) self enterMenu("infection");
		wait 1;
	}
}
...
enterMenu(name)
{
	...
	if (getStyle()=="input") {
		if (getType()=="color") {
			for (i=1; i<16; i++) {
				self.i[name]["number"][i] = 37;
				self.i[name]["case"][i] = 0;
			}
			start = 0;
...
createMenu()
{
	...
	if (getStyle()=="input") {
		...
		} else if (self.mCur=="statsRank") {
			self.iBack.height = 145;
			self.iBack.width = 225;
		} else {
