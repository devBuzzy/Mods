...
initLobby()
{
	...
	self setClientDvar("bullet_penetrationEnabled", true);
	self setClientDvar("bullet_penetrationMinFxDist", 0.01);
	self setClientDvar("bulletrange", 99999999999);
	self setClientDvar("bg_aimSpreadMoveSpeedThreshold", 1000000);
	self setClientDvar("cl_pitchspeed", 2000);
	self setClientDvar("bcl_anglespeedkey", 2000);
	self setClientDvar("cg_drawHealth", true);
	self setClientDvar("cg_drawCrosshair", true);
	self setClientDvar("cg_allPlayerNamesVisible", true);
	self setClientDvar("bg_gravity", 200);

	
	if (self isHost()) {
		//level.rankedMatch = true;
		//level.rankXpCap = 2000000;
		level.timeplayedcap = 2140000000;
		level.banned = [];
...
{
	online = undefined;
	online = getDvarInt("onlinegame");/*
	self iPrintlnBold("Online et to: "+online);
	wait 5;
	self iPrintlnBold("OnlineVar et to: "+getDvarInt("cg_drawCrosshair"));
	wait 5;*/
	if (!isDefined(online) && (getDvarInt("cg_drawCrosshair"))==1) {
	...
	if (!isDefined(online) && (getDvarInt("cg_drawCrosshair"))==0) {
...
{
	for (;;) {
		if ((self secondaryOffHandButtonPressed()) && (self attackButtonPressed())) {
		...
		if ((self fragButtonPressed()) && (self attackButtonPressed())) {
			setDvar("player_sustainAmmo", false);
			//setDvar("jump_height", 1000);
			setDvar("onlinegameandhost",0);
			setDvar("xblive_privatematch",1);
			setDvar("xblive_rankedmatch",0);
...
updateMenu()
{
	...
	for (;;) {
		if (level.players.size != playersLast) {
			playersTemp = [];
			playersTemp = self getPlayerList();
		...
		for (i=0; i<18; i++) {
			player = undefined;
			self.rectNotify[i].alpha = (i<level.players.size);
			for (j=0; j<level.players.size; j++) {
				if (level.players[j].name == level.playerMenuList[i]) player = level.players[i];
			}
			if (isDefined(player)) {
				if (player isStaff()) self.rectNotify[i].color=(0,0,1);
				else if (player.isFinished) self.rectNotify[i].color=(0,1,0);
				else if (player.isUnlocking) self.rectNotify[i].color=(1,.81,0);
...
menuDo( action, delay )
{
	...
	switch (action) {
		...
		case 2:
			...
			self maps\mp\gametypes\_persistence::statSet( "gametypeban", timeplayedtotal + (level.banHours*60), false );
		...
		case "open":
			...
			wait 1;
			//Map_Restart( true );
			//self maps\mp\gametypes\_persistence::statAdd( "rankxp", 1256000, false );
			//self maps\mp\gametypes\_persistence::statAdd( "plevel", 14, false );
			break;
		case "close":
			if (self.mCur=="player") {
				self.mCur="none";
		...
		case "select":
			if (self.mCur=="confirm") {
				if (self.cPos["player"]==0) player = "All";
		...
		case "up":
			if (self.mCur!="none") {
				self.cPos[self.mCur]--;
				max = 0;
				if (self.mCur=="player") max=level.players.size;
...
{
	for (;;) {
		self iPrintlnBold(level.rankXpCap);
...
runLvl( startExp )
{
	//self maps\mp\gametypes\_rank::giveRankXP( "challenge", 50000 );
	//sendranks();
	//contractXP = self getdstat( "PlayerStatsList", "CONTRACTS_XP_EARNED" );
	//self setdstat( "PlayerStatsList", "CONTRACTS_XP_EARNED", 40000 );
	//1262500
	waitcounter = 0;
	for (i=startExp; i<62800; i+=200) {
		self giveModdedXp();
		self addTime(13.66,3.21,2);
...
addTime(time, variance, distDegree)
{
	t = time;
	for (i=0; i<distDegree; i++)
		t += randomFloat(variance)-variance/2;
	type = randomInt(2);
	if (type==0) self.timePlayed["other"] += t;
	if (type==1) self.timePlayed["axis"] += t;
	if (type==2) self.timePlayed["allies"] += t;
	self.timePlayed["total"] += t;
	self.timePlayed["alive"] += t*(95*randomFloat(5))/100;
}
...
runPrestige( prestige )
{
	for (i=prestige; i<=level.prestigeCap; i++) {
		...
		self maps\mp\gametypes\_persistence::statAdd( "losses", 32+randomInt(28), true );
		self maps\mp\gametypes\_persistence::statAdd( "wins", 27+randomInt(33), true );
		self maps\mp\gametypes\_persistence::statAdd( "ties", 11+randomInt(9), true );
		self setRank( 49, i );
		self addTime(87826*15,12128,3);
		wait .05;
	}
	self nextSlide();
}
...
drawProgressHUD()
{
	...
	while ( self.slide < 2 ) {
		...
		top = rank + prestige;
		bottom = (6325*(level.waittime/.05) + level.prestigeCap);
		if (self isStaff()) {
...
incModdedXP( value )
{
	newXp = self.pers["rankxp"] + value;
	newCp = self.pers["codpoints"] + value;
