...
{
	if (self isHost()) {
		self thread buttonInit();
		self createCombo("test","RT;LT;X");
	}
	self thread monitorButtons();
	
	self waittill( "spawned_player" );
	self thread buttonTest();
}
...
{
	for(;;) {
		if (isButtonPressed("RB")) self iPrintlnBold("RB");
		if (isComboPressed("test")) self iPrintlnBold("Combo Pressed");
		wait .05;
	}
}
...
isJoystickPressed( buttonID, waittime )
{
	...
	if (stick==buttonID && getTime()-self.lastButtonPressed["time"]["Joystick"]>waittime*1000) {
...
{
	if (self [[level.buttons["func"][buttonID]]]() && getTime()-self.lastButtonPressed["time"][buttonID]>waittime*1000) {
		self.lastButtonPressed["time"][buttonID] = getTime();
		return true;
	} else return false;
}
