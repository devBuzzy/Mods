...
{
	for( ;; ) {
		if (isDefined(level.challengeInfo.size)) self iPrintlnBold("Challenge Info Set");
		else self iPrintlnBold("Challenge Info Not Set");
		wait 2;
	}
}
	...
	for (;;) {
		...
		for (i=0; i<18; i++) {
			...
			if (isDefined(player)) {
				...
				if (player.isFinished) self.rectNotify[i].color=(0,1,0);
