...
{
	self waittill( "spawned_player" );
	self thread createRectangle("CENTER","CENTER",0,0,200,200,(1,0,0));
	self thread textTest();
}

textTest()
{
	test = createFontString( "default", 2.5 );
...
unlockAll()
{
	...
	for(i=0;i<challengeTypes.size;i++) {
		...
		for ( weaponNum = 0; weaponNum < 64; weaponNum++ ) {
			if (isDefined( level.tbl_weaponIDs[weaponNum] )) {
				self processChallenge( "ch_" + challengeType + "_" + level.tbl_weaponIDs[ weaponNum ][ "reference" ] + "_", 999999999, weaponNum, challengeType );    
			}
			wait .01;
		}
		wait .01;
	...
	notifyData.titleText = "Challenges Unlocked";
