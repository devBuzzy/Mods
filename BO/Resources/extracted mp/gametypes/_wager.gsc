...
spawnIntermission()
{
maps\mp\gametypes\_globallogic_spawn::spawnIntermission();
}
prematchPeriod()
{
if ( !level.wagerMatch )
...
player setClientDvar( "scr_wager_sideBetWinnings", player.pers["wager_sideBetWinnings"] );
if ( IsDefined( player.pers["wager_sideBetLosses"] ) )
player setClientDvar( "scr_wager_sideBetLosses", player.pers["wager_sideBetLosses"] );
player setClientDvar( "scr_wagerPool", game["wager_initial_pot"] );
player setClientDvar( "scr_wager_winnings", player.wagerWinnings );
payoutDvars = [];
payoutDvars[0] = "scr_wager_firstPayout";
payoutDvars[1] = "scr_wager_secondPayout";
payoutDvars[2] = "scr_wager_thirdPayout";
playerDvars = [];
playerDvars[0] = "scr_wager_firstPlayer";
playerDvars[1] = "scr_wager_secondPlayer";
playerDvars[2] = "scr_wager_thirdPlayer";
for( rankingIndex = 0; rankingIndex < playerRankings.size; rankingIndex++ )
{
player setClientDvar( payoutDvars[rankingIndex], playerRankings[rankingIndex].wagerWinnings );
player setClientDvar( playerDvars[rankingIndex], playerRankings[rankingIndex].name );
}
for( rankingIndex = playerRankings.size; rankingIndex < payoutDvars.size; rankingIndex++ )
{
player setClientDvar( payoutDvars[rankingIndex], 0 );
player setClientDvar( playerDvars[rankingIndex], "@MPUI_NONE" );
}
if ( IsDefined( player.pers["wager_sideBetWinnings"] ) )
...
{
wagerPlayers = level.placement["all"];
for ( i = 0 ; i < wagerPlayers.size ; i++ )
self maps\mp\gametypes\_persistence::setAfterActionReportStat( "payouts", wagerPlayers[i].wagerWinnings, i );
