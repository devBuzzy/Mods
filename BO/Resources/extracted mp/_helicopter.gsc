...
{
chopper thread heli_protect( startNode, protectLocation, hardpointType );
chopper.chaffcount = 1;
}
else
{
...
for( ;; )
{
self waittill( "stinger_fired_at_me", missile, weap );
_incomingMissile(missile);
heli_fire_chaff( missile );
wait( 1.0 );
}
}
_incomingMissile( missile )
...
heli_protect( startNode, protectDest, hardpointType )
{
self endon( "death" );
self notify( "flying");
self endon( "flying" );
...
protectDest = ( protectDest[0], protectDest[1], noFlyZoneHeight );
currentDest = protectDest;
startTime = gettime();
...
assist_level_value = int( floor( (damagedone/level.heli_maxhealth) / 25 ) );
